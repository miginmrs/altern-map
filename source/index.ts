...
export function alternMap<T, O extends ObservableInput<any>>(project: (value: T, index: number) => O): OperatorFunction<T, ObservedValueOf<O>>;
/** @deprecated resultSelector is no longer supported, use inner map instead */
export function alternMap<T, O extends ObservableInput<any>>(project: (value: T, index: number) => O, resultSelector: undefined): OperatorFunction<T, ObservedValueOf<O>>;
/** @deprecated resultSelector is no longer supported, use inner map instead */
export function alternMap<T, R, O extends ObservableInput<any>>(project: (value: T, index: number) => O, resultSelector: (outerValue: T, innerValue: ObservedValueOf<O>, outerIndex: number, innerIndex: number) => R): OperatorFunction<T, R>;
...
export function alternMap<T, R, O extends ObservableInput<any>>(
  ...
  return (source: Observable<T>) => source.lift(new AlternMapOperator(project));
}
...
class AlternMapOperator<T, R> implements Operator<T, R> {
  ...
  call(subscriber: Subscriber<R>, source: any): any {
    return source.subscribe(new AlternMapSubscriber(subscriber, this.project));
  }
}
